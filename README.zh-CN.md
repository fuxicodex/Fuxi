...
- **自带密钥，或直接登录** —— 任意 OpenAI 兼容 / Gemini / Bedrock / Vertex
  API Key，或通过 FuXi OAuth 登录（无需密钥）。
- **本地优先、保护隐私** —— 配置、凭据、会话与记忆都留在你的设备
  `~/.fuxi/` 下；提示词与代码直达你选择的提供商。
- **永久免费** —— 单个静态二进制，无运行时依赖，个人、团队、企业均无需
  许可费用。
