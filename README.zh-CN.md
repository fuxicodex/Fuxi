...
FuXi 是一个快速、自包含的 AI 开发者终端 —— 一个由丰富的 TUI 驱动的编程智能体。
它能读懂你的代码库、编辑文件、运行命令、驱动工具，并在多个 LLM 提供商之间进行
成本感知的路由与自动故障转移。使用 Go 编写，以单个静态二进制文件分发，
无运行时依赖。
...
FuXi 是一个单一的 Go module。入口位于 `cmd/fuxi`；系统主体位于
`internal/` 下 —— 智能体引擎、TUI、工具、提供商、MCP、会话与记忆服务、
智能路由器，以及权限系统。
