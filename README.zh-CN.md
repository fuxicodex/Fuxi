...
- **零内容上传、本地优先** —— FuXi 从不收集、存储或保留你的代码、提示词与
  对话，也从不要求你上传它们；配置、凭据、会话与记忆留在你的设备 `~/.fuxi/`
  下。自带密钥时内容直达你的提供商；使用托管模型时，内容仅为服务该次请求而传输。
- **自动更新** —— 后台版本检查加一条 `fuxi update`，替换二进制前先做校验
  和验证。
- **面向开发者** —— FuXi 面向开发者与专业用户，不面向不满 14 周岁的儿童；
  14–17 岁的用户需有父母或监护人参与。
...
| [架构](docs/architecture.zh-CN.md) | 执行底座、持久化、路由、多窗口协同与可扩展性 |
...
| [服务条款](security-privacy/TERMS_OF_SERVICE.zh-CN.md) | 使用 FuXi 的协议 |
| [隐私控制](security-privacy/PRIVACY_CONTROLS.zh-CN.md) | 哪些数据留在本地，以及如何更改网络/隐私设置 |
...
**你必须依照适用于你的法律使用 FuXi** —— 包括你所在国家与地区的法律、数据
保护法律，以及适用于你工作的行业规则。FuXi 的技术能力不等于许可：若某用途在
你所在处属违法，你不得使用 FuXi 从事该用途。

禁止用途与年龄要求（面向开发者；不面向不满 14 周岁儿童；14–17 岁需父母或
监护人参与）见[使用政策](security-privacy/policies/ACCEPTABLE_USE.zh-CN.md)。
