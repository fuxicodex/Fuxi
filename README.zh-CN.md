...
转移。FuXi 使用 Go 语言开发，交付为单个静态二进制文件，无运行时依赖。

FuXi 以对标 Claude 最顶级编程智能体为目标。凭借 思考 → 行动 → 验证 的
智能体循环与智能路由，把任意支持 OpenAPI 的模型的推理能力提升到 Claude
最高版本的级别 —— 以更低的成本交付：轻量模型处理简单任务，每个模型都能
发挥出远超其原生基准的表现。
...
![FuXi 与其他 AI 编程智能体对比](docs/comparison.svg)

![AI 编程智能体定位](docs/positioning.svg)

### 我们自己跑过的一次对比

为了让自己的说法站得住脚，我们也做了一次受控对比：FuXi 驱动一个 OpenAPI
兼容的推理模型，对阵 Claude Code 驱动一个顶级 Claude 模型，各自通过原生
客户端，在 11 个完全相同的编码任务上（缺陷修复、功能实现、重构、测试生成、
...
已知局限都记录在 [`benchmark/REPORT.zh-CN.md`](benchmark/REPORT.zh-CN.md)，
可供核实或自行复现。

有一点需要坦率说明：这是一套自测的小规模任务集，并非第三方基准，且衡量
的是**智能体循环**而非模型裸分。请把它当作一个参考数据点，而非结论性
标题。
