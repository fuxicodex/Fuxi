...
- **零内容上传** —— FuXi 从不收集、存储或保留你的代码、提示词与对话，也从不
  要求你上传它们。自带密钥时内容直达你的提供商；使用托管模型时，内容仅为服务
  该次请求而传输 —— 不留存、不用于训练。
...
注册仅处理最少账户数据 —— **你的代码与对话绝不被 FuXi 收集、存储或保留**
（零内容上传）。
...
FuXi 的**最低使用年龄为 13 岁**；未满 18 岁者，须有父母或监护人参与，并在
法律要求时提供同意。完整要求见
