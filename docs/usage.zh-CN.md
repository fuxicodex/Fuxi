...
- **不绑定提供商** —— 可使用任意 OpenAI 兼容端点、Gemini、Bedrock/Vertex
  或其他 OpenAI 兼容提供商，或登录后使用 FuXi 托管模型。
- **零内容上传（默认）** —— FuXi 默认不收集、存储或保留你的代码、提示词与对话，也从不
  把上传它们作为使用条件。自带密钥时内容直达你的提供商；使用托管模型时，内容仅为服务
  该次请求而传输 —— 不留存、不用于训练。仅当你显式启用 `send_conversations`
  （默认关闭）时，对话内容才会发送给 FuXi（见隐私控制）。
...
`fuxi login` 注册或认证你的 FuXi 账号并授予访问权限。用 `fuxi logout`
退出登录。

无交互/CI 场景用 `fuxi setup-token`，它会打印一个可导出为
`FUXI_OAUTH_TOKEN` 的令牌。

注册仅处理最少账户数据 —— **默认情况下，你的代码与对话绝不被 FuXi 收集、存储或保留**
（零内容上传；仅当你启用 `send_conversations` 时例外）。

### 2. 连接模型（可选）

登录后即可使用 FuXi 托管模型。若想改用自己的提供商，请自带密钥：通过环境
变量设置提供商 API Key，或直接编写 `~/.fuxi/config.yaml`。
...
**你必须依照适用于你的法律使用 FuXi** —— 你所在国家/地区的法律、数据保护
法律，以及适用于你工作的行业规则。FuXi 的技术能力不等于许可：若某用途在你
所在处属违法，你不得使用 FuXi 从事该用途。

简言之，你不得使用 FuXi 实施或协助违法活动、伤害儿童、造成损害或破坏、
未授权访问、侵犯隐私、欺骗或欺诈、制造非法内容、击穿安全机制以实施上述损害、
将 AI 输出冒充为经核实的专业建议，或滥用服务。
...
- **Hooks、skills 与插件** —— 可扩展且支持热重载，并提供官方插件。
