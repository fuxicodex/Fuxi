...
Install from the official FuXi marketplace, then reload plugins in the same session:

    /plugin install fuxi-security@fuxi-plugins
    /reload-plugins

If FuXi reports that the marketplace is not found, run `/plugin marketplace add fuxicodex/plugins` first, then retry.
...
- FuXi with this plugin installed
