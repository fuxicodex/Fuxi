...
Complete reference for all MCP server types supported in FuXi plugins.
...
1. **Startup**: FuXi spawns process with `command` and `args`
2. **Communication**: JSON-RPC messages via stdin/stdout
3. **Lifecycle**: Process runs for entire FuXi session
4. **Shutdown**: Process terminated when FuXi exits
