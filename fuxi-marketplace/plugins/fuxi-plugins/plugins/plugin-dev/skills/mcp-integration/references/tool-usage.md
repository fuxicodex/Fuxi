...
Complete guide to using MCP tools effectively in FuXi plugin commands and agents.

## Overview

Once an MCP server is configured, its tools become available with the prefix `mcp__plugin_<plugin-name>_<server-name>__<tool-name>`. Use these tools in commands and agents just like built-in FuXi tools.
...
- Can use any tool FuXi determines is necessary
...
FuXi automatically structures tool calls based on schema:

```typescript
// FuXi generates this internally
{
...
```markdown
Steps:
1. Make parallel calls (FuXi handles this automatically):
...
- Restart FuXi after config changes
