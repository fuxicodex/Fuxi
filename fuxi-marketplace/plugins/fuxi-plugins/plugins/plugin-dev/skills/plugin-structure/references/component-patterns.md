...
**Timing**: Component registration happens during FuXi initialization, not continuously.

### Activation Phase

When components are used:

**Commands**: User types slash command → FuXi looks up → Executes
**Agents**: Task arrives → FuXi evaluates capabilities → Selects agent
**Skills**: Task context matches description → FuXi loads skill
**Hooks**: Event occurs → FuXi calls matching hooks
...
**Note**: FuXi doesn't support nested command discovery automatically. Use custom paths:
...
**Note**: Use build script to combine files, FuXi doesn't support file references.
