...
**You send to FuXi with the system prompt above:**
```
Create an agent configuration based on this request: "I need an agent that reviews pull requests for code quality issues"
```

**FuXi returns JSON (note: prose `whenToUse`, "When to invoke" section in `systemPrompt`):**
```json
{
...
2. Feed to FuXi with this system prompt
