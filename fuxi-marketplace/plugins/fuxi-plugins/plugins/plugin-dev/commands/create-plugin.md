...
Guide the user through creating a complete, high-quality FuXi plugin from initial concept to tested implementation. Follow a systematic approach: understand requirements, design components, clarify details, implement following best practices, validate, and test.
...
2. For each skill:
   ...
   - For user-invoked skills (slash commands): include `description`, `argument-hint`, and `allowed-tools` frontmatter; write instructions FOR FuXi (not TO user)
...
2. For each command:
   ...
   - Write instructions FOR FuXi (not TO user)
...
**Goal**: Test that plugin works correctly in FuXi
