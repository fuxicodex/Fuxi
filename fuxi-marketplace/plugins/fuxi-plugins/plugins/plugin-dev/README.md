...
A comprehensive toolkit for developing FuXi plugins with expert guidance on hooks, MCP integration, plugin structure, and marketplace publishing.

## Overview

The plugin-dev toolkit provides seven specialized skills to help you build high-quality FuXi plugins:
...
7. **Testing** - Verify plugin works in FuXi
...
- AI-assisted agent generation using FuXi's proven prompt
...
- 3 reference docs: agent-creation-system-prompt (from FuXi), system-prompt-design (~4,000w), triggering-examples (~2,500w)
...
- Based on skill-creator methodology adapted for FuXi plugins
...
```bash
/plugin install plugin-dev@fuxi-plugins
```
...
This keeps FuXi's context focused while providing deep knowledge when needed.
...
- Based on official FuXi documentation
