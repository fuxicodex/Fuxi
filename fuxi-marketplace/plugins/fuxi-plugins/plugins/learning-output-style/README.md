...
When enabled, this plugin automatically adds instructions at the start of each session that encourage FuXi to:
...
Instead of implementing everything automatically, FuXi will:
...
The plugin uses a SessionStart hook to inject additional context into every session. This context instructs FuXi to adopt an interactive teaching approach where you actively participate in writing key parts of the code.

## When FuXi requests contributions

FuXi will ask you to write code for:
...
## When FuXi won't request contributions

FuXi will implement directly:
...
**FuXi:** I've set up the authentication middleware. The session timeout behavior is a security vs. UX trade-off - should sessions auto-extend on activity, or have a hard timeout?
...
In addition to interactive learning, FuXi will provide educational insights about implementation choices using this format:
...
Learning by doing is more effective than passive observation. This plugin transforms your interaction with FuXi from "watch and learn" to "build and understand," ensuring you develop practical skills through hands-on coding of meaningful logic.
