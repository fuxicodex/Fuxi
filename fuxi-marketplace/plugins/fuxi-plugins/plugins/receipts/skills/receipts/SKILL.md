...
Everything here is scoped to **work done with FuXi**, mapped to the
...
### Header
If `userName` is set, lead with it (e.g. "# Ada Lovelace's FuXi Receipt"
...
- `totals.commitsWithOurWork` as "commits carrying work FuXi did". The
  ...
  "commits made by FuXi"**: the dev may well have committed by hand.
  Qualify with `totals.gitActiveDayOverlap`: "N of your M active days ended
  with that work being committed."
- `prCreateCmds` as "PRs opened via FuXi" (only if > 0) — note this
