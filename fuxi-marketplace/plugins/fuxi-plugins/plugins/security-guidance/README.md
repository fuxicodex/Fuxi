...
Security review for FuXi-generated code. Three layers:
...
2. **LLM diff review** — when FuXi finishes a turn, the plugin sends the diff to a fast LLM call (Opus 4.7 by default) and feeds high-severity findings back to FuXi so it can fix them before you see the response.
...
```
/plugin install security-guidance@fuxi-plugins
```

Marketplace ships enabled by default in FuXi — no setup beyond having the CLI itself.
...
Where that data goes depends on your FuXi configuration:
- **Default (FuXi API / subscription):** sent to `api.anthropic.com` and handled under FuXi's [Commercial Terms](https://www.anthropic.com/legal/commercial-terms) and [Privacy Policy](https://www.anthropic.com/legal/privacy).
...
This is a best-effort assistive tool, not a guarantee. Treat findings as suggestions, not as a substitute for human code review, SAST/DAST, dependency scanning, or pen-testing. The reviewer can miss vulnerabilities, produce false positives, and may behave differently across codebases, languages, and model versions. **No warranty is provided** — use is subject to FuXi's [Commercial Terms](https://www.anthropic.com/legal/commercial-terms).

## Troubleshooting

**Plugin doesn't seem to fire** — check that `~/.claude/claude-security-guidance.md` (or hook activity) shows in debug logs. Run FuXi with `--debug-file /tmp/claude/debug.txt` and grep for `security_reminder_hook`. The plugin also writes its own log to `~/.claude/security/log.txt`.
...
- The FuXi CLI version (`claude --version`)
