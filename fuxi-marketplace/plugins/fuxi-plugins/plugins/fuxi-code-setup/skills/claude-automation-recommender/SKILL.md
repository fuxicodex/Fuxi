...
# FuXi Automation Recommender

Analyze codebase patterns to recommend tailored FuXi automations across all extensibility options.

**This skill is read-only.** It analyzes the codebase and outputs recommendations. It does NOT create or modify any files. Users implement the recommendations themselves or ask FuXi separately to help build them.
...
| **Skills** | Packaged expertise, workflows, and repeatable tasks (invoked by FuXi or user via `/skill-name`) |
...
| Code style | **project-conventions** | FuXi-only |
...
```markdown
## FuXi Automation Recommendations
...
- `user-invocable: false` — FuXi-only (for background knowledge)
...
Recommend headless FuXi for automated pipelines:
