...
MCP (Model Context Protocol) servers extend FuXi's capabilities by connecting to external tools and services.
...
### context7
**Best for**: Projects using popular libraries/SDKs where you want FuXi to code with up-to-date documentation
...
**Value**: FuXi fetches live documentation instead of relying on training data, reducing hallucinated APIs and outdated patterns.
...
**Value**: FuXi can interact with your running app, take screenshots, fill forms, and verify UI behavior.
...
**Value**: FuXi can query tables, manage auth, and interact with Supabase storage directly.
...
**Value**: FuXi can introspect the live deployment (tables, function specs, env vars, logs) and execute queries/mutations against it via tools like `tables`, `function-spec`, `data`, `run-once-query`, `logs`, `env list/set/get`. Run via `npx convex mcp start`.
...
**Value**: FuXi can create issues, review PRs, check workflow runs, and manage releases.
...
**Value**: FuXi can investigate Sentry issues, find root causes, and suggest fixes.
...
**Value**: FuXi remembers project context, decisions, and patterns across conversations.
