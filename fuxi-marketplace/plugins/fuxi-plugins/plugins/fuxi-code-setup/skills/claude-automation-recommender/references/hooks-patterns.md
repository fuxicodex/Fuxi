...
Hooks automatically run commands in response to FuXi events. They're ideal for enforcement and automation that should happen consistently.
...
Notification hooks run when FuXi sends notifications. Use matchers to filter by notification type.

### Permission Alerts
| Matcher | Use Case |
|---------|----------|
| `permission_prompt` | Alert when FuXi requests permissions |
...
| Matcher | Use Case |
|---------|----------|
| `idle_prompt` | Alert when FuXi is waiting for input (60+ seconds idle) |

**Recommend**: Play sound or send notification when FuXi needs attention
**Value**: Know when FuXi is ready for your input
    ...
    "Notification": [
      ...
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "osascript -e 'display notification \"FuXi is waiting\" with title \"FuXi\"'"
          }
        ]
      }
    ]
...
|---------|---------------|
| `permission_prompt` | FuXi needs permission for a tool |
| `idle_prompt` | FuXi waiting for input (60+ seconds) |
