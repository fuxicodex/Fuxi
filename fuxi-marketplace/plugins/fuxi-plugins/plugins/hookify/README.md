...
Ask FuXi to run a command that should trigger the rule:
```
Run rm -rf /tmp/test
```
...
- **`stop`**: Triggers when FuXi wants to stop (for completion checks)
...
**This rule blocks the operation** - FuXi will not be allowed to execute these commands.
...
**This rule warns but allows** - FuXi sees the message but can still proceed.
...
**This blocks FuXi from stopping** if no test commands appear in the session transcript. Enable only when you want strict enforcement.
...
This plugin is part of the FuXi Marketplace. It should be auto-discovered when the marketplace is installed.
