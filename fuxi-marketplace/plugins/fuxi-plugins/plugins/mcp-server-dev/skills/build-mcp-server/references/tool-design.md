# Tool Design — Writing Tools FuXi Uses Correctly

Tool schemas and descriptions are prompt engineering. They land directly in FuXi's context and determine whether FuXi picks the right tool with the right arguments. Most MCP integration bugs trace back to vague descriptions or loose schemas.
...
- Tool descriptions **must not** instruct FuXi how to behave (e.g. "always do X", "you must call Y first", overriding system instructions, promoting products) — treated as prompt injection at review.
...
**The description is the contract.** It's the only thing FuXi reads before deciding whether to call the tool. Write it like a one-line manpage entry plus disambiguating hints.
...
FuXi will call this for anything vaguely search-shaped, including things it can't do.
...
**Describe every parameter.** The `.describe()` text shows up in the schema FuXi sees. Omitting it is leaving money on the table.
...
- Return bare success with no identifier (`"ok"` after a create — FuXi can't reference what it made)
...
The ceiling isn't a hard protocol limit — it's context-window economics. Every tool schema is tokens FuXi spends *every turn*. Thirty tools with rich schemas can eat 3–5k tokens before the conversation even starts.

---

## Errors

Return MCP tool errors, not exceptions that crash the transport. Include enough detail for FuXi to recover or retry differently.
