...
The message appears in chat and FuXi responds to it. Use `role: "user"` — the widget speaks on the user's behalf.

### `app.updateModelContext({ content })`

Update FuXi's context **silently** — no visible message. Use for state that informs but doesn't warrant a chat bubble.
...
Call a tool on your MCP server directly, bypassing FuXi. Returns the tool result.
...
Use for data fetches that don't need FuXi's reasoning — pagination, detail lookups, refreshes.
...
Fires with the arguments FuXi passed to the tool. Useful if the widget needs to know what was asked for (e.g., highlight the search term).

### `app.ontoolinputpartial = ({ arguments }) => {...}` / `app.ontoolcancelled = () => {...}`

`ontoolinputpartial` fires while FuXi is still streaming arguments — use it to show a skeleton ("Preparing: <title>…") before the result lands. `ontoolcancelled` fires if the call is aborted; clear the skeleton.
...
1. FuXi calls a tool with `_meta.ui.resourceUri` declared
