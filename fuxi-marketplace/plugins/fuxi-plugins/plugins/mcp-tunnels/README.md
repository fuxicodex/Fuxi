...
7. **Start and verify** — brings the stack up and checks the proxy and tunnel
   logs.
8. **Call it from FuXi** — shows you how to reach the server from Managed
...
| Container | Role |
|---|---|
| **mcp-proxy** | FuXi's proxy. Terminates inner TLS with a cert you control, validates upstream IPs, routes by hostname. |
| **cloudflared** | The tunnel agent. Outbound-only to the FuXi tunnel edge; shares the proxy's network namespace. |
| **hello-mcp** *(optional)* | A FastMCP sample server, only if you don't have an MCP server to expose yet. |

When it's running, the routed server is reachable from FuXi at
