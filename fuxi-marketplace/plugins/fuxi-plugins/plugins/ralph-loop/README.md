...
Implementation of the Ralph Wiggum technique for iterative, self-referential AI development loops in FuXi.
...
This plugin implements Ralph using a **Stop hook** that intercepts FuXi's exit attempts:

```bash
# You run ONCE:
/ralph-loop "Your task description" --completion-promise "DONE"

# Then FuXi automatically:
# 1. Works on the task
# 2. Tries to exit
...
- FuXi's previous work persists in files
- Each iteration sees modified files and git history
- FuXi autonomously improves by reading its own past work in files
...
**Location**: `~/.fuxi/plugins/marketplaces/fuxi-plugins/ralph-wiggum/<hash>/hooks/hooks.json`
...
Run `/help` in FuXi for detailed command reference and examples.
