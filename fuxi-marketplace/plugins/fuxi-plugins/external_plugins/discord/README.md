...
Connect a Discord bot to your FuXi with an MCP server.

When the bot receives a message, the MCP server forwards it to FuXi and provides tools to reply, react, and edit messages.
...
Install the plugin:
```
/plugin install discord@fuxi-plugins
```
...
```sh
fuxi --channels plugin:discord@fuxi-plugins
```

**7. Pair.**

With FuXi running from the previous step, DM your bot on Discord — it replies with a pairing code. If the bot doesn't respond, make sure your session is running with `--channels`. In your FuXi session:
...
Pairing is for capturing IDs. Once you're in, switch to `allowlist` so strangers don't get pairing-code replies. Ask FuXi to do it, or `/discord:access policy allowlist` directly.
