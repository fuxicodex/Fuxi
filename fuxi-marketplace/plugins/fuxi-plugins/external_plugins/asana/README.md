...
[Asana](https://asana.com) is a work management platform for tasks, projects, and goals. This plugin connects FuXi to Asana's **V2 MCP server** so you can create and manage tasks, search projects, update assignments, and track progress directly from your terminal.
...
You only need to do this once. The fastest path is to run **`/asana-setup`** inside FuXi and follow the printed steps, or do it manually:
...
3. Under **OAuth**, add this exact **Redirect URL**:
   ...
   (This is FuXi's local OAuth callback. It is `localhost` by design — FuXi runs on your machine and catches the authorization code on a local listener. It must match the `--callback-port` you use below.)
4. Copy your **Client ID** and **Client Secret**.

### 2. Add the Asana V2 server to FuXi
...
- `--client-secret` with no value makes FuXi prompt for the secret and store it securely in your OS keychain (never on disk).
- `--callback-port 8080` must match the port in the redirect URL you registered.

### 3. Authenticate and verify

1. The next time the `asana` server is used, FuXi opens your browser for Asana consent. Approve it.
...
3. Try it: ask FuXi to "list my Asana workspaces" or "show my assigned tasks."
...
- [Connecting MCP clients to Asana's V2 server (FuXi)](https://developers.asana.com/docs/connecting-mcp-clients-to-asanas-v2-server)
