#!/usr/bin/env bash
# Claude Code (native client) multi-dimensional benchmark driver (11 scenarios).
# Usage: run_claude.sh
...
RESULT="$BENCH/result-claude-$MODEL_NAME.json"
echo "{" > "$RESULT"
...
run_scenario() {
  ...
  local log="$BENCH/log-claude-$MODEL_NAME-$dir.log"
  (cd "$repo" && claude -p --dangerously-skip-permissions "$prompt") > "$log" 2>&1
  local cx=$?
  local final
  ...
  echo "  baseline: $base"; echo "  claude exit: $cx"; echo "  final: $final"
  python3 - "$RESULT" "$dir" "$base" "$final" "$cx" <<'PYEOF'
import json, sys
path, d, base, final, cx = sys.argv[1:6]
data = {}
try: data = json.load(open(path))
except Exception: pass
data[d] = {"baseline": base, "final": final, "claude_exit": int(cx)}
