...
| **🎯 任务** | **🧠 对比系统** | **✅ 总体通过** |
| **`15 微任务 + 4 大型项目`** | **`2`** | **`100%`** |
| 确定性 · 客观评分 | FuXi + deepseek-v4-flash vs Claude Code + claude-opus-5 | 零失败 |

</div>

> **摘要：** 本报告呈现一项受控、可复现的评估，对比两款 AI 编码智能体——
> **FuXi**（驱动 OpenAPI 兼容推理模型 `deepseek-v4-flash`）与 **Claude Code**
...
| 系统 | 客户端 | 版本 | 模型 |
|---|---|---|---|
| FuXi | `fuxi -p` | `0.15`（预计发布） | `deepseek-v4-flash`（OpenAPI 兼容） |
...
| **模型身份** | `deepseek-v4-flash` 与 `claude-opus-5` 为配置/代理声明 ID，未独立核实 |
...
FuXi 驱动 `deepseek-v4-flash` 在微观与大型项目智能体编码任务上，均与 Claude Code
